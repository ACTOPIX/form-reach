...
function wp_action_form_include($id) {
	...
	$siteKey = esc_attr( get_option('wpaf_key_site') );

	wp_enqueue_script('wp-action-form', plugin_dir_url(__FILE__) . 'js/wp-action-form.js');
	wp_localize_script('wp-action-form', 'wpaf_vars', array(
		'wpaf_key_site' => get_option('wpaf_key_site'),
		'wapf_recaptcha_switch' => get_option('wapf_recaptcha_switch')
	));
	...
	?>
		...
		<form <?php if ($wp_stored_meta_front['wpaf_whatsapp_switch'][0] == 1){?>id="action_form_whatsapp"<?php }else{ ?> id="action_form_mail" <?php } ?> accept-charset="UTF-8" name="action_form_mail" method="post" action="javascript:void(0)">
			...
			<div id="wpaf_mail_submit" <?php if ($wp_stored_meta_front['wpaf_whatsapp_switch'][0] == 1){?>style="display:none"<?php }else{ ?> style="display:block" <?php } ?>>
				<button type="submit" id="submit" name="wpaf_mail_submit" class="btn btn-primary mb-3 mt-3 <?php if(esc_attr(get_option('wpaf_recaptcha_switch')) === '1') { ?>g-recaptcha<?}?>">
			...
			<div id="wpaf_whatsapp_submit" <?php if ($wp_stored_meta_front['wpaf_whatsapp_switch'][0] == 1){?>style="display:block"<?php }else{ ?> style="display:none" <?php } ?>>
				<button type="submit" id ="whatsapp" name="wpaf_whatsapp_submit" class="btn btn-success mb-3 mt-3 <?php if(esc_attr(get_option('wpaf_recaptcha_switch')) === '1') { ?>g-recaptcha<?}?>">
