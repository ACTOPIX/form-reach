...
function init_db_action_form() {
	...
	$formulaire = $wpdb->prefix . "formulaire";
		...
		if( $wpdb->get_var( "show tables like '$formulaire'" ) != $formulaire) {
			...
			$sql .= " `objectif` varchar(1000) NOT NULL, ";
...
function wp_action_form_include($id) {
	...
	?>
		...
		<form <?php if ($wp_stored_meta_front['wpaf_whatsapp_switch'][0] == 1){?>id="action_form_whatsapp"<?php }else{ ?> id="action_form_mail" <?php } ?> accept-charset="UTF-8" name="action_form_mail" method="post" <?php if ($wp_stored_meta_front['wpaf_whatsapp_switch'][0] == 1){?>action="<?echo plugin_dir_url(__FILE__)?>process/whatsapp.php"<?php }else{ ?> action="javascript:void(0)" <?php } ?>>
