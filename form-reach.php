...
// Registers and enqueues 'form-reach.js' with the handle 'form-reach' on the frontend of the website.
// It depends on the jQuery library and is hooked to the 'wp_enqueue_scripts' action.
function bbx_enqueue_scripts() {
        wp_register_script( 'form-reach', plugin_dir_url(__FILE__) . 'js/form-reach.js', 'jquery', '1.0' );
...
function form_reach_include($id) {
	...
	?>	
		<?php if(esc_attr(get_option('fr_recaptcha_switch')) === '1') { ?>	
			<script src="https://www.google.com/recaptcha/api.js?render=<?php echo esc_attr( get_option('fr_key_site') ); ?>"></script>
		<?php } ?>
			...
			<div class="container">	
				<div class="row">	
					<form <?php if ($wp_stored_meta_front['fr_whatsapp_switch'][0] == 1){?>id="form_reach_whatsapp"<?php }else{ ?> id="form_reach_mail" <?php } ?> accept-charset="UTF-8" name="form_reach_mail" method="post" action="javascript:void(0)">
						...
						<?if ($wp_stored_meta_front['fr_whatsapp_switch'][0] == 1){ echo do_shortcode($wp_stored_meta_front["fr_whatsapp_form_content"][0]); }else{ echo do_shortcode($wp_stored_meta_front["fr_email_form_content"][0]); } ?>

						<div id="fr_mail_submit" <?php if ($wp_stored_meta_front['fr_whatsapp_switch'][0] == 1){?>style="display:none"<?php }else{ ?> style="display:block" <?php } ?>>
							<button type="submit" name="fr_mail_submit" class="btn mb-3 mt-3 <?php if(esc_attr(get_option('fr_recaptcha_switch')) === '1') { ?>g-recaptcha<?}?>" style="background-color: <?php echo $wp_stored_meta_front['fr_email_submit_color'][0]; ?>;">
								<div id="submitContent">
									<svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="<?php echo $wp_stored_meta_front['fr_email_text_color'][0];?>" class="bi bi-envelope" viewBox="0 0 16 16">
										<path d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4Zm2-1a1 1 0 0 0-1 1v.217l7 4.2 7-4.2V4a1 1 0 0 0-1-1H2Zm13 2.383-4.708 2.825L15 11.105V5.383Zm-.034 6.876-5.64-3.471L8 9.583l-1.326-.795-5.64 3.47A1 1 0 0 0 2 13h12a1 1 0 0 0 .966-.741ZM1 11.105l4.708-2.897L1 5.383v5.722Z"/>
									</svg>
									<span style="color: <?php echo $wp_stored_meta_front['fr_email_text_color'][0];?>;"><?php echo $wp_stored_meta_front['fr_email_submit'][0];?></span>
								</div>
								<div id="spinner" class="spinner-border spinner-border-sm text-white" style="display:none"></div>
							</button>
						</div>

						<div id="fr_whatsapp_submit" <?php if ($wp_stored_meta_front['fr_whatsapp_switch'][0] == 1){?>style="display:block"<?php }else{ ?> style="display:none" <?php } ?>>
							<button type="submit" name="fr_whatsapp_submit" class="btn mb-3 mt-3 <?php if(esc_attr(get_option('fr_recaptcha_switch')) === '1') { ?>g-recaptcha<?}?>" style="background-color: <?php echo $wp_stored_meta_front['fr_whatsapp_submit_color'][0]; ?>;">
								<div id="submitContentWhatsapp">
									<svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="<?php echo $wp_stored_meta_front['fr_whatsapp_text_color'][0];?>" class="bi bi-whatsapp" viewBox="0 0 16 16">
									...
									<span style="color: <?php echo $wp_stored_meta_front['fr_whatsapp_text_color'][0];?>;"><?php echo $wp_stored_meta_front['fr_whatsapp_submit'][0];?></span>
								</div>
						...
						<div style="display:none">
							<input type="hidden" name="fr_container_post" value="<?echo $id['id'];?>">
					...
					<div id="success_message" class="alert alert-success" style="display:none">
						...
						<?php if ($wp_stored_meta_front['fr_whatsapp_switch'][0] == 1){ echo $wp_stored_meta_front['fr_whatsapp_success'][0]; }else{ echo $wp_stored_meta_front['fr_email_success'][0]; } ?>
					</div>
					...
					<div id="error_message" class="alert alert-danger" style="display:none">
						...
						<?php if ($wp_stored_meta_front['fr_whatsapp_switch'][0] == 1){ echo $wp_stored_meta_front['fr_whatsapp_error'][0]; }else{ echo $wp_stored_meta_front['fr_email_error'][0]; } ?>
					</div>
				</div>
			</div>
		</section>
