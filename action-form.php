...
function bbx_enqueue_scripts() {
        wp_register_script( 'action-form', plugin_dir_url(__FILE__) . 'js/action-form.js', 'jquery', '1.0' );
		wp_enqueue_script( 'jquery' );
		wp_enqueue_script( 'action-form');
    }
			...
			<div class="container">	
				<div class="row">	
					<form <?php if ($wp_stored_meta_front['wpaf_whatsapp_switch'][0] == 1){?>id="action_form_whatsapp"<?php }else{ ?> id="action_form_mail" <?php } ?> accept-charset="UTF-8" name="action_form_mail" method="post" action="javascript:void(0)">
						...
						<?if ($wp_stored_meta_front['wpaf_whatsapp_switch'][0] == 1){ echo do_shortcode($wp_stored_meta_front["wpaf_whatsapp_form_content"][0]); }else{ echo do_shortcode($wp_stored_meta_front["wpaf_email_form_content"][0]); } ?>
